...
uint8 TS_n_Comp_initVar = 0u;

/* Internal functions definitoin */
static void TS_n_Comp_trimAdjust(uint8 nibble) ;

/* static TS_n_Comp_backupStruct  TS_n_Comp_backup; */
#if (CY_PSOC5A)
    static TS_n_Comp_LOWPOWER_BACKUP_STRUCT  TS_n_Comp_lowPowerBackup;
#endif /* CY_PSOC5A */

/* variable to decide whether or not to restore the control register in 
   Enable() API for PSoC5A only */
#if (CY_PSOC5A)
    static uint8 TS_n_Comp_restoreReg = 0u;
#endif /* CY_PSOC5A */


/*******************************************************************************
* Function Name: TS_n_Comp_Init
********************************************************************************
*
* Summary:
...
void TS_n_Comp_Init(void) 
{
    ...
    TS_n_Comp_SetSpeed(TS_n_Comp_DEFAULT_SPEED);

    /* Set default Hysteresis */
    #if ( TS_n_Comp_DEFAULT_HYSTERESIS == 0u )
        TS_n_Comp_CR |= TS_n_Comp_HYST_OFF;
    #else
        TS_n_Comp_CR &= (uint8)(~TS_n_Comp_HYST_OFF);
    #endif /* TS_n_Comp_DEFAULT_HYSTERESIS == 0u */
    /* Power down override feature is not supported for PSoC5A. */
    #if (CY_PSOC3 || CY_PSOC5LP)
        /* Set default Power Down Override */
        #if ( TS_n_Comp_DEFAULT_PWRDWN_OVRD == 0u )
            TS_n_Comp_CR &= (uint8)(~TS_n_Comp_PWRDWN_OVRD);
        #else 
            TS_n_Comp_CR |= TS_n_Comp_PWRDWN_OVRD;
        #endif /* TS_n_Comp_DEFAULT_PWRDWN_OVRD == 0u */
    #endif /* CY_PSOC3 || CY_PSOC5LP */
    
    /* Set mux always on logic */
    TS_n_Comp_CR |= TS_n_Comp_MX_AO;

    /* Set default sync */
    TS_n_Comp_CLK &= (uint8)(~TS_n_Comp_SYNCCLK_MASK);
    #if ( TS_n_Comp_DEFAULT_BYPASS_SYNC == 0u )
        TS_n_Comp_CLK |= TS_n_Comp_SYNC_CLK_EN;
    #else
        TS_n_Comp_CLK |= TS_n_Comp_BYPASS_SYNC;
    #endif /* TS_n_Comp_DEFAULT_BYPASS_SYNC == 0u */
}


/*******************************************************************************
* Function Name: TS_n_Comp_Enable
********************************************************************************
*
* Summary:
...
{
    TS_n_Comp_PWRMGR |= TS_n_Comp_ACT_PWR_EN;
    TS_n_Comp_STBY_PWRMGR |= TS_n_Comp_STBY_PWR_EN;
     
     /* This is to restore the value of register CR which is saved 
    in prior to the modification in stop() API */
    #if (CY_PSOC5A)
        if(TS_n_Comp_restoreReg == 1u)
        {
            TS_n_Comp_CR = TS_n_Comp_lowPowerBackup.compCRReg;

            /* Clear the flag */
            TS_n_Comp_restoreReg = 0u;
        }
    #endif /* CY_PSOC5A */
}
...
*
* Global variables:
*  TS_n_Comp_initVar: Is modified when this function is called for the 
...
void TS_n_Comp_Start(void) 
{
    ...
    if ( TS_n_Comp_initVar == 0u )
    {
        TS_n_Comp_Init();
        
        TS_n_Comp_initVar = 1u;
    }   
...
void TS_n_Comp_Stop(void) 
{
    ...
    TS_n_Comp_PWRMGR &= (uint8)(~TS_n_Comp_ACT_PWR_EN);
    TS_n_Comp_STBY_PWRMGR &= (uint8)(~TS_n_Comp_STBY_PWR_EN);
    
    #if (CY_PSOC5A)
        /* Enable the variable */
        TS_n_Comp_restoreReg = 1u;

        /* Save the control register before clearing it */
        TS_n_Comp_lowPowerBackup.compCRReg = TS_n_Comp_CR;
        TS_n_Comp_CR = TS_n_Comp_COMP_REG_CLR;
    #endif /* CY_PSOC5A */
}


/*******************************************************************************
* Function Name: TS_n_Comp_SetSpeed
********************************************************************************
*
* Summary:
...
void TS_n_Comp_SetSpeed(uint8 speed) 
{
    ...
    TS_n_Comp_CR = (TS_n_Comp_CR & (uint8)(~TS_n_Comp_PWR_MODE_MASK)) |
                           (speed & TS_n_Comp_PWR_MODE_MASK);

    /* Set trim value for high speed comparator */
    if(speed == TS_n_Comp_HIGHSPEED)
    {
        /* PSoC5A */
        #if (CY_PSOC5A)
            TS_n_Comp_TR = TS_n_Comp_HS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            TS_n_Comp_TR0 = TS_n_Comp_HS_TRIM_TR0;
            TS_n_Comp_TR1 = TS_n_Comp_HS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
    else
    {
    /* PSoC5A */
        #if (CY_PSOC5A)
            TS_n_Comp_TR = TS_n_Comp_LS_TRIM_TR0;
        #endif /* CY_PSOC5A */
        
        /* PSoC3, PSoC5LP or later */
        #if (CY_PSOC3 || CY_PSOC5LP) 
            TS_n_Comp_TR0 = TS_n_Comp_LS_TRIM_TR0;
            TS_n_Comp_TR1 = TS_n_Comp_LS_TRIM_TR1;
        #endif /* CY_PSOC3 || CY_PSOC5LP */
    }
...
/*******************************************************************************
* Function Name: TS_n_Comp_GetCompare
********************************************************************************
*
* Summary:
...
{
    return( TS_n_Comp_WRK & TS_n_Comp_CMP_OUT_MASK);
}


/*******************************************************************************
* Function Name: TS_n_Comp_trimAdjust
********************************************************************************
*
* Summary:
...
static void TS_n_Comp_trimAdjust(uint8 nibble) 
{
    ...
    cmpState = TS_n_Comp_WRK & TS_n_Comp_CMP_OUT_MASK;
    ...
    {    
        ...
        if ( cmpState != 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                TS_n_Comp_TR |= TS_n_Comp_CMP_TRIM1_DIR;
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                TS_n_Comp_TR0 |= TS_n_Comp_CMP_TR0_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    else
    {
        ...
        if ( cmpState == 0u )
        {
            /* PSoC5A */
            #if (CY_PSOC5A)
                TS_n_Comp_TR |= TS_n_Comp_CMP_TRIM2_DIR; 
            #endif /* CY_PSOC5A */
            
            /* PSoC3, PSoC5LP or later */
            #if (CY_PSOC3 || CY_PSOC5LP)
                TS_n_Comp_TR1 |= TS_n_Comp_CMP_TR1_DIR;
            #endif /* CY_PSOC3 || CY_PSOC5LP */
        }
    }
    ...
    for ( trimCnt = 0u; trimCnt < trimCntMax; trimCnt++ )
	{
        ...
        if ((TS_n_Comp_WRK & TS_n_Comp_CMP_OUT_MASK) != cmpState)
        {
...
/*******************************************************************************
* Function Name: TS_n_Comp_ZeroCal
********************************************************************************
*
* Summary:
...
uint16 TS_n_Comp_ZeroCal(void) 
{
    ...
    tmpSW0 = TS_n_Comp_SW0;
    tmpSW2 = TS_n_Comp_SW2;
    tmpSW3 = TS_n_Comp_SW3;

     /* Clear routing for inP, retain routing for inN */
    TS_n_Comp_SW0 = 0x00u;
    TS_n_Comp_SW2 = 0x00u;
    TS_n_Comp_SW3 = tmpSW3 & (uint8)(~TS_n_Comp_CMP_SW3_INPCTL_MASK);
    ...
    */
    tmpCR = TS_n_Comp_CR;
    TS_n_Comp_CR |= (TS_n_Comp_CAL_ON | TS_n_Comp_HYST_OFF);
    
    /* Write default low values to trim register - no offset adjust */
    /* PSoC5A */
    #if (CY_PSOC5A)
        TS_n_Comp_TR = TS_n_Comp_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        TS_n_Comp_TR0 = TS_n_Comp_DEFAULT_CMP_TRIM;
        TS_n_Comp_TR1 = TS_n_Comp_DEFAULT_CMP_TRIM;
    #endif /* CY_PSOC3 || CY_PSOC5LP */
	
	/* Two phase trim - slow modes, one phase trim - for fast */ 
    if ( (TS_n_Comp_CR & TS_n_Comp_PWR_MODE_MASK) == TS_n_Comp_PWR_MODE_FAST)
    {
        TS_n_Comp_trimAdjust(0u);
    }
    else /* default to trim for fast modes */
    {
        TS_n_Comp_trimAdjust(1u);
		TS_n_Comp_trimAdjust(0u);
    }
    ...
    TS_n_Comp_SW0 = tmpSW0;
    TS_n_Comp_SW2 = tmpSW2;
    TS_n_Comp_SW3 = tmpSW3;
    
    /* PSoC5A */
    #if (CY_PSOC5A)
        return (uint16) TS_n_Comp_TR;
    #endif /* CY_PSOC5A */
    
    /* PSoC3, PSoC5LP or later */
    #if (CY_PSOC3 || CY_PSOC5LP)
        return ((uint16)((uint16)TS_n_Comp_TR1 << 8u) | (TS_n_Comp_TR0));        
    #endif /* CY_PSOC3 || CY_PSOC5LP */
}


/*******************************************************************************
* Function Name: TS_n_Comp_LoadTrim
********************************************************************************
*
* Summary:
...
void TS_n_Comp_LoadTrim(uint16 trimVal) 
{
    ...
    /*******************************************************************************
    * Function Name: TS_n_Comp_PwrDwnOverrideEnable
    ********************************************************************************
    *
    * Summary:
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************
    * Function Name: TS_n_Comp_PwrDwnOverrideDisable
    ********************************************************************************
    *
    * Summary:
